...
# 每display_step轮打一次结果
display_step = 50
max_source_sentence_length = 200 # data_process里面计算过
...
def encoder_layer(rnn_inputs, rnn_size, rnn_num_layers,
    ...
    @param source_sequence_len: 代码句子序列的长度
    @param source_vocab_size: 代码词典的大小
...
def decoder_layer_inputs(target_data, target_vocab_to_int, batch_size):
    ...
    @param target_data: 注释数据的tensor
    @param target_vocab_to_int: 注释数据的词典到索引的映射
...
def decoder_layer_train(encoder_states, decoder_cell, decoder_embed,
    ...
    @param encoder_states: Encoder端编码得到的上下文向量
    @param decoder_cell: Decoder端
    @param decoder_embed: Decoder端词向量嵌入后的输入
    @param target_sequence_len: 注释文本的长度
    @param max_target_sequence_len: 注释文本的最大长度
...
def decoder_layer(encoder_output,encoder_states, decoder_inputs, target_sequence_len, 
    ...
    @param target_vocab_to_int: 注释单词到token id的映射
    @param target_vocab_size: 注释词典的大小
...
def seq2seq_model(input_data, target_data, batch_size,
    ...
    @param batch_size: batch size
    @param source_sequence_len: 代码的长度
    @param target_sequence_len: 注释的长度
    @param max_target_sentence_len: 注释的最大句子长度
    @param source_vocab_size: 代码词典的大小
    @param target_vocab_size: 注释词典的大小
...
with tf.Session(graph=train_graph) as sess:
    ...
    writer = tf.summary.FileWriter(os.path.join("logs"), sess.graph)
    #100轮次的迭代
    ...
    saver.save(sess, os.path.join("tmp","checkpoints","model_nl.ckpt"))
    print('Model Trained and Save to {}'.format(os.path.join("tmp","checkpoints","model_nl.ckpt")))
